...
# Clone and build barforge.com
RUN /usr/bin/git clone https://github.com/fooforge/fooforge.com.git /var/www/barforge.com/
RUN /bin/bash -lc 'cd /var/www/barforge.com; bundle'
RUN /bin/bash -lc 'cd /var/www/barforge.com; jekyll build'

ADD .docker/nginx-barforge.com /etc/nginx/sites-available/barforge.com
RUN /bin/bash -c '/bin/ln -s /etc/nginx/sites-available/barforge.com /etc/nginx/sites-enabled/barforge.com'
